...
if ($_REQUEST['do'] == 'counters')
{
	...
	print_input_row($vbphrase['number_of_users_to_process_per_cycle_gmaintenance'], 'perpage', 2000);
...
if ($_REQUEST['do'] == 'updateattachments')
{
	...
	if (!$continue)
	{
		print_stop_message2('updated_blog_attachments_successfully', 'blog_admin');
	}

	$finishat = $vbulletin->GPC['startat'] + $vbulletin->GPC['perpage'];
	echo '<p>' . $vbphrase['updating_blog_attachments'] . '</p>';

	$attachments = $assertor->getRows('getAttachments', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'attachmentid' => $vbulletin->GPC['startat'],
	...
	foreach ($attachments as $attachment)
	{
		...
		$dataman = new vB_Datamanager_AttachmentFiledata($vbulletin, vB_DataManager_Constants::ERRTYPE_STANDARD);
		...
		{
			$assertor->delete('blog_attachment', array('attachmentid' => $attachment[attachmentid]));
			if ($vbulletin->options['blogattachfile'])
			{
			...
			if (($newpagetext = preg_replace('#\[attach(.*)\]' . $attachment['attachmentid'] . '\[/attach\]#siU', '[attach\\1]' . $attachmentid . '[/attach]', $attachment['pagetext'])) != $attachment['pagetext'])
			{
				...
				$assertor->update('blog_text', array('pagetext' => $newpagetext), array('blogtextid' => $attachment['blogtextid']));
			}

			$assertor->insert('replaceBlogAttachment', array(
					vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
	...
	if ($checkmore = $assertor->getRow('getAttachmentId', array('attachmentid' => $finishat)))
	{
		$args = array();
		parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
		$args['do'] = 'updateattachments';
		$args['startat'] = $finishat;
		$args['pp'] = $vbulletin->GPC['perpage'];
		print_cp_redirect2('blog_admin', $args);

		echo "<p><a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=updateattachments&amp;startat=$finishat&amp;pp=" . $vbulletin->GPC['perpage'] . "\">" . $vbphrase['click_here_to_continue_processing'] . "</a></p>";
	}
	else
	{
		// Drop table if it truly is empty!
		if (!$assertor->getRow('blog_attachment', array()))
		{
			$assertor->assertQuery('dropBlogAttachment', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED));
			$assertor->assertQuery('dropBlogAttachmentViews', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED));
		}
		print_stop_message2('updated_blog_attachments_successfully', 'blog_admin');
	}
}
...
{
	$vbulletin->input->clean_array_gpc('r', array(
		'cantview' => vB_Cleaner::TYPE_BOOL,
		'cantpost' => vB_Cleaner::TYPE_BOOL
	));
	...
	$blogs = $assertor->getRows('blog', array(
			vB_dB_Query::CONDITIONS_KEY => array(
					array('field' => 'blogid', 'value' => $vbulletin->GPC['startat'], 'operator' => vB_dB_Query::OPERATOR_GTE),
					array('field' => 'blogid', 'value' => $finishat, 'operator' => vB_dB_Query::OPERATOR_LT)
			)
	), 'blogid');
	...
	if (($checkmore = $assertor->getRow('blog', array(
			vB_dB_Query::CONDITIONS_KEY => array(
					array('field' => 'blogid', 'value' => $finishat, 'operator' => vB_dB_Query::OPERATOR_GTE)
			)))))
	{
		$args = array();
		parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
		...
		print_cp_redirect2('blog_admin', $args);
		echo "<p><a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . http_build_query($args, '', '&amp;') . "\">" . $vbphrase['click_here_to_continue_processing'] . "</a></p>";
	}
	else
	{
		print_stop_message2('updated_blog_entries_successfully', 'blog_admin');
	}
}
...
if ($_REQUEST['do'] == 'updateuser')
{
	...
	$users = $assertor->getRows('blogAdminUsers', array(
			'userid' => $vbulletin->GPC['startat'],
			vB_dB_Query::PARAM_LIMIT => $vbulletin->GPC['perpage']
	));
	...
	if (($checkmore = $assertor->getRow('user', array(
			vB_dB_Query::CONDITIONS_KEY => array(
	...
	))))
	{
		...
		$args["pp"] = 		$vbulletin->GPC['perpage'];
		print_cp_redirect2('blog_admin', $args);
		echo "<p><a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=updateuser&amp;startat=$finishat&amp;pp=" . $vbulletin->GPC['perpage'] . "\">" . $vbphrase['click_here_to_continue_processing'] . "</a></p>";
	}
	else
	{
		print_stop_message2('updated_blog_users_successfully', 'blog_admin');
	}
}
...
{
	$assertor->assertQuery('truncateBlogTextparsed');
	print_stop_message2('blog_cache_emptied', 'blog_admin');
}
...
if ($_POST['do'] == 'rebuildcounters')
{
	...
	$assertor->assertQuery('dropTableBlogTrackbackCount', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'tablename' => $tablename
	));

	$assertor->assertQuery('createTableBlogTrackbackCount', array(
			'tablename' => $tablename
	));

	$assertor->update('updateBlogTrackbackCountVisible', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'tablename' => $tablename
	));

	$assertor->update('updateBlogTrackbackCountModeration', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'tablename' => $tablename
	));

	$assertor->asserQuery('dropTableBlogTrackbackCount', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'tablename' => $tablename
	));
	...
	print_stop_message2('blog_counters_rebuilt', 'blog_admin');
}
...
if ($_REQUEST['do'] == 'moderators')
{
	...
	$countmods = 0;
	$supergroups = $assertor->getRows('getSuperGroups', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
	...
	if (count($supergroups))
	{
		foreach ($supergroups as $supergroup)
		{
			...
			echo "\n\t<li><b><a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&u=$supergroup[userid]\">$supergroup[username]</a></b><span class=\"smallfont\"> (" . construct_link_code($vbphrase['edit_permissions'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=editglobal&amp;u=$supergroup[userid]") . ") - " . $vbphrase['last_online'] . " <span class=\"$onlinecolor\">" . $lastonline . "</span></span></li>\n";
		}
	}
	else
	{
	...
	$countmods = 0;
	$moderators = $assertor->getRows('getBlogAdminModerators', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED));
	if (count($moderators))
	{
		foreach ($moderators as $moderator)
		{
			...
			echo " <span class=\"smallfont\">(" .
				construct_link_code($vbphrase['edit'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=editmod&blogmoderatorid=$moderator[blogmoderatorid]") .
				construct_link_code($vbphrase['remove'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=removemod&blogmoderatorid=$moderator[blogmoderatorid]") .
			")</span>";
		}
		echo "\t\t</ul>\n\t\t</ul>\n\t</li>\n\t</ul>\n";
	}
...
{
	$vbulletin->input->clean_array_gpc('r', array(
		'blogmoderatorid'	=> vB_Cleaner::TYPE_INT,
	...
	{
		$moderator = $assertor->getRow('getBlogAdminModerator', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
	...
	else
	{
		...
		$moderator = $assertor->getRow('getBlogModerator', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'blogmoderatorid' => $vbulletin->GPC['blogmoderatorid']
		));
...
if ($_POST['do'] == 'updatemod')
{
	$vbulletin->input->clean_array_gpc('p', array(
		'modusername'     => vB_Cleaner::TYPE_NOHTML,
		'moderator'       => vB_Cleaner::TYPE_ARRAY,
		'modperms'        => vB_Cleaner::TYPE_ARRAY,
		'blogmoderatorid' => vB_Cleaner::TYPE_UINT,
	...
	{ // update
		$vbulletin->db->query_write(fetch_query_sql($vbulletin->GPC['moderator'], 'blog_moderator', "WHERE blogmoderatorid=" . $vbulletin->GPC['blogmoderatorid']));

		print_stop_message2(array('saved_moderator_x_successfully',  $vbulletin->GPC['modusername']), 'blog_admin', array('do'=>'moderators'));
	}
	else
	{ // insert
		if (($userinfo = $assertor->getRow('getBlogAdminUserinfo', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'mod_type' => $vbulletin->GPC['type'],
				'username' => $vbulletin->GPC['modusername']
		))))
		{
			if ($userinfo['bmuserid'])
			{
				print_stop_message2('user_already_moderator');
			}

			$vbulletin->GPC['moderator']['userid'] = $userinfo['userid'];
			$vbulletin->GPC['moderator']['type'] = $vbulletin->GPC['type'];
			$vbulletin->db->query_write(fetch_query_sql($vbulletin->GPC['moderator'], 'blog_moderator'));

			if ($vbulletin->GPC['type'] == 'normal')
			{
				$dataman = new vB_Datamanager_Blog_User($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);
...
{
	$vbulletin->input->clean_array_gpc('r', array('blogmoderatorid' => vB_Cleaner::TYPE_UINT));
...
$vbulletin->input->clean_array_gpc('p', array('blogmoderatorid' => vB_Cleaner::TYPE_UINT));
...
if ($_POST['do'] == 'killmod')
{
	...
	$getuserid = $assertor->getRow('getBlogAdminUserid', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
			'blogmoderatorid' => $vbulletin->GPC['blogmoderatorid']
	));
	if (!$getuserid)
	...
	else
	{
		...
		$dataman = new vB_Datamanager_Blog_User($vbulletin, vB_DataManager_Constants::ERRTYPE_SILENT);
		$dataman->set_existing($userinfo);
		$dataman->set('isblogmoderator', 0);
		$dataman->save();

		$assertor->delete('blog_moderator', array(
				'blogmoderatorid' => $vbulletin->GPC['blogmoderatorid']
		));

		print_stop_message2('deleted_moderator_successfully', 'blog_admin', array('do'=>'moderators'));
	}
}
...
if ($_REQUEST['do'] == 'dolist')
{
	...
	{
		print_stop_message2('start_date_after_end');
	}

	if (!$vbulletin->GPC['userid'] AND $vbulletin->GPC['user'])
	{
		if (!$user = $assertor->getRow('user', array('username' => $vbulletin->GPC['user'])))
		{
			print_stop_message2(array('could_not_find_user_x',  $vbulletin->GPC['user']));
		}
	...
	do
	{
		...
		$params['start'] = $start;
		$params['perpage'] = $vbulletin->GPC['perpage'];
		$entries = $assertor->getRows('getBlogAdminEntries', $params);
		/*)"
		...
		");*/
		list($totalentries) = $assertor->getRow('getFoundRows', array());
	...
	if (count($entries))
	{
		if ($vbulletin->GPC['pagenumber'] != 1)
		{
			...
			$firstpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"&laquo; " . $vbphrase['first_page'] . "\" onclick=\"window.location='blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=1'\">";
			$prevpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"&lt; " . $vbphrase['prev_page'] . "\" onclick=\"window.location='blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$prv'\">";
		}

		if ($vbulletin->GPC['pagenumber'] != $totalpages)
		{
			$nxt = $vbulletin->GPC['pagenumber'] + 1;
			$nextpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" . $vbphrase['next_page'] . " &gt;\" onclick=\"window.location='blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$nxt'\">";
			$lastpage = "<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" . $vbphrase['last_page'] . " &raquo;\" onclick=\"window.location='blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . $vbulletin->GPC['orderby'] . "&page=$totalpages'\">";
		}
		...
		$headings = array();
		$headings[] = "<a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . "username\" title=\"" . $vbphrase['order_by_username'] . "\">" . $vbphrase['user_name'] . "</a>";
		$headings[] = "<a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . "title\" title=\"" . $vbphrase['order_by_title'] . "\">" . $vbphrase['title'] . "</a>";
		$headings[] = "<a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=dolist" . $args . "\" title=\"" . $vbphrase['order_by_date'] . "\">" . $vbphrase['date'] . "</a>";
		...
		{
			$cell = array();
			$cell[] = "<a href=\"user.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&amp;u=$entry[userid]\"><b>$entry[username]</b></a>";
			...
			$cell[] = construct_link_code($vbphrase['delete'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=deleteentry&blogid=$entry[blogid]" . $args . $vbulletin->GPC['orderby'], false, '', true);
...
if ($_POST['do'] == 'killentry')
{
	...
	if ($bloginfo = fetch_bloginfo($vbulletin->GPC['blogid']))
	{
		$blogman = new vB_Datamanager_Blog($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);
		...
		print_stop_message2('deleted_entry_successfully', 'blog_admin', $args);
	}
	else
	{
		print_stop_message2('no_matches_found_gerror', 'blog_admin', $args);
	}
}
...
if ($_REQUEST['do'] == 'rebuildprofilepic')
{
	...
	{
		print_stop_message2('your_version_no_image_support');
	}
	...
	{
		$firstpic = $assertor->getRow('getFirstCustomProfilePic', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED));
	...
	if ($vbulletin->GPC['startat'])
	{
		...
		echo '<p>' . construct_phrase($vbphrase['calculating_profile_pic_dimensions'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=rebuildprofilepic&startat=" . $vbulletin->GPC['startat'] . "&pp=" . $vbulletin->GPC['perpage']) . '</p>';

		$image = vB_Image::instance();

		$pictures = $assertor('getUserCustomProfilePics', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'userid' => $vbulletin->GPC['startat'],
				vB_dB_Query::PARAM_LIMIT => $vbulletin->GPC['perpage']
		));
		...
		foreach ($pictures as $picture)
		{
			...
			$imageinfo = $image->fetchImageInfo($filename);
			if ($imageinfo[0] AND $imageinfo[1])
			{
				$dataman = new vB_Datamanager_Userpic_Profilepic($vbulletin, vB_DataManager_Constants::ERRTYPE_SILENT, 'userpic');
		...
		if (($checkmore = $assertor->getRow('customprofilepic', array(
				vB_dB_Query::CONDITIONS_KEY => array(
				array('field' => 'userid', 'value' => $finishat, 'operator' => vB_dB_Query::OPERATOR_GTE)
		)))))//"SELECT userid FROM " . TABLE_PREFIX . "customprofilepic WHERE userid >= $finishat LIMIT 1")))
		{
			$args = array();
			parse_str(vB::getCurrentSession()->get('sessionurl'),$args);
			$args["do"] =		"rebuildprofilepic";
			$args["startat"] = 	$finishat;
			$args["pp"] = 		$vbulletin->GPC['perpage'];
			print_cp_redirect2('blog_admin', $args);
			echo "<p><a href=\"blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=rebuildprofilepic&amp;startat=$finishat&amp;pp=" . $vbulletin->GPC['perpage'] . "\">" . $vbphrase['click_here_to_continue_processing'] . "</a></p>";
		}
		else
		{
			print_stop_message2('updated_profile_pictures_successfully', 'blog_admin');
		}
	}
	else
	{
...
if ($_REQUEST['do'] == 'listfe')
{
	...
	function js_jump(id, obj)
	{
		...
		switch (task)
		{
			case 'modifyfe': window.location = "blog_admin.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=modifyfe&featureid=" + id; break;
			case 'killfe': window.location = "blog_admin.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=removefe&featureid=" + id; break;
	...
	$entries = $assertor->getRows('getFeaturedBlogEntries', array(vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED));
	...
	print_table_footer(9, (count($entries) ? '<input type="submit" class="button" value="' . $vbphrase['save_display_order'] . '" accesskey="s" tabindex="1" />' : '') . construct_button_code($vbphrase['add_new_featured_entry'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=modifyfe"));
}
...
{
	$vbulletin->input->clean_array_gpc('r', array(
		'featureid' => vB_Cleaner::TYPE_UINT,
		'type'      => vB_Cleaner::TYPE_NOHTML,
	));
	...
	{
		$entry = $assertor->getRow('getFeaturedBlogEntryById', array(
		...
		if (!$entry)
		{
			print_stop_message2(array('invalid_x_specified',  'featureid'));
		}
...
if ($_POST['do'] == 'doorderfe')
{
	...
	{
		$entries = $assertor->getRows('blog_featured', array());
		foreach ($entries as $entry)
		{
			if ($entry['displayorder'] != $vbulletin->GPC['displayorder']["$entry[featureid]"])
			{
				$assertor->update('blog_featured', array(
						'displayorder' => $vbulletin->GPC['displayorder']["$entry[featureid]"]
				), array(
	...
	print_stop_message2('saved_display_order_successfully', 'blog_admin', array('do'=>'listfe'));
}
...
if ($_POST['do'] == 'updatefe')
{
	$vbulletin->input->clean_array_gpc('p', array(
		'featureid'    => vB_Cleaner::TYPE_UINT,
		'type'         => vB_Cleaner::TYPE_NOHTML,
		'pusergroupid' => vB_Cleaner::TYPE_UINT,
		'susergroupid' => vB_Cleaner::TYPE_UINT,
		'start'        => vB_Cleaner::TYPE_ARRAY_UINT,
		'end'          => vB_Cleaner::TYPE_ARRAY_UINT,
		'refresh'      => vB_Cleaner::TYPE_UINT,
		'timespan'     => vB_Cleaner::TYPE_STR,
		'blogid'       => vB_Cleaner::TYPE_UINT,
	...
	{
		$entry = $assertor->getRow('blog_featured', array(
				'featureid' => $vbulletin->GPC['featureid']
		));
		if (!$entry)
		{
	...
	if ($values['type'] == 'specific')
	{
		...
		if (!$bloginfo)
		{
			print_stop_message2(array('invalid_x_specified',  $vbphrase['blogid']));
		}
		else
		{
	...
	else
	{
		...
		{
			$userinfo = $assertor->getRow('user', array(
	...
	{
		$assertor->insert('blog_featured', array(
				'userid' => $values['userid'],
				'blogid' => $values['blogid'],
				'pusergroupid' => $values['pusergroupid'],
				'susergroupid' => $values['susergroupid'],
	...
	print_stop_message2('updated_featured_entry_successfully', 'blog_admin', array('do'=>'listfe'));
}

// ##################### Start Remove Feature Entry ###################################
if ($_REQUEST['do'] == 'removefe')
{
	$vbulletin->input->clean_array_gpc('r', array('featureid' => vB_Cleaner::TYPE_UINT));
...
if ($_POST['do'] == 'killfe')
{
	...
	$getfeatureid = $assertor->getRow('blog_featured', array(
			'featureid' => $vbulletin->GPC['featureid']
	));
	...
	{
		$assertor->delete('blog_featured', array(
				'featureid' => $vbulletin->GPC['featureid']
		));

		build_featured_entry_datastore();

		print_stop_message2('deleted_featured_entry_successfully', 'blog_admin', array('do'=>'listfe'));
	}
}
...
if ($_REQUEST['do'] == 'listcat')
{
	...
	function js_jump(id, obj)
	{
		...
		switch (task)
		{
			case 'modifycat': window.location = "blog_admin.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=modifycat&blogcategoryid=" + id; break;
			case 'killcat': window.location = "blog_admin.php?<?php echo vB::getCurrentSession()->get('sessionurl_js'); ?>do=removecat&blogcategoryid=" + id; break;
			default: return false; break;
		}
	}
	</script>
	...
	print_table_footer(3, (!empty($vbulletin->vbblog['categorycache']["0"]) ? '<input type="submit" class="button" value="' . $vbphrase['save_display_order'] . '" accesskey="s" tabindex="1" />' : '') . construct_button_code($vbphrase['add_new_blog_category'], "blog_admin.php?" . vB::getCurrentSession()->get('sessionurl') . "do=modifycat"));
}
...
if ($_POST['do'] == 'doordercat')
{
	...
	if (!empty($vbulletin->GPC['displayorder']))
	{
		$categories = $assertor->getRows('blog_category', array('userid' => 0));
		foreach ($categories as $category)
		{
			if ($category['displayorder'] != $vbulletin->GPC['displayorder']["$category[blogcategoryid]"])
			{
				$assertor->update('blog_category', array(
						'displayorder' => $vbulletin->GPC['displayorder']["$category[blogcategoryid]"]
				), array(
	...
	print_stop_message2('saved_display_order_successfully', 'blog_admin', array('do'=>'listcat'));
}
...
{
	$vbulletin->input->clean_array_gpc('r', array(
		'blogcategoryid' => vB_Cleaner::TYPE_UINT,
	));
	...
	if ($vbulletin->GPC['blogcategoryid'])
	{
		...
		$category = $assertor->getRow('getCategoryTitleDesc', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'title' => $title,
				'desc' => $desc,
		...
		if (!$category)
		{
			print_stop_message2(array('invalid_x_specified',  $vbphrase['blog_category']));
		}
	...
	$trans_link = "phrase.php?" . vB::getCurrentSession()->get('sessionurl') . "do=edit&fieldname=vbblogcat&t=1&varname="; // has varname appended
	...
	print_textarea_row($vbphrase['description_gcpglobal'] . ($desc ? '<dfn>' . construct_link_code($vbphrase['translations'], $trans_link . $desc, 1)  . '</dfn>' : ''),  'description', $category['description']);
...
if ($_POST['do'] == 'updatecat')
{
	...
	fetch_ordered_categories(0);
	$dataman = new vB_Datamanager_Blog_Category($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);

	if ($vbulletin->GPC['blogcategoryid'])
	{
		if (!($categoryinfo = $assertor->getRow('blog_category', array(
				'blogcategoryid' => $vbulletin->GPC['blogcategoryid'],
				'userid' => 0
		))))
		{
			print_stop_message2(array('invalid_x_specified',  'categoryid'));
		}
	...
	print_stop_message2('saved_blog_category_successfully', 'blog_admin', array('do'=>'listcat'));
}
...
function is_subcategory_of($categoryid, $parentid)
{
	...
	if (is_array($vbulletin->vbblog['icategorycache']["0"]["$categoryid"]))
	{
		foreach ($vbulletin->vbblog['icategorycache']["0"]["$categoryid"] AS $curcategoryid => $category)
		{
			...
			{
				print_stop_message2('cant_parent_category_to_child');
			}
		}
	}
...
{
	$vbulletin->input->clean_array_gpc('r', array('blogcategoryid' => vB_Cleaner::TYPE_UINT));

	print_delete_confirmation('blog_category', $vbulletin->GPC['blogcategoryid'], 'blog_admin', 'killcat', '', 0, '', 'title', 'blogcategoryid');
}
...
if ($_POST['do'] == 'killcat')
{
	...
	$categoryinfo = $assertor->getRow('blog_category', array(
			'blogcategoryid' => $vbulletin->GPC['blogcategoryid'],
			'userid' => 0
	));
	...
	if (!$categoryinfo)
	{
		print_stop_message2(array('invalid_x_specified',  'blogcategoryid'));
	}
	else
	{
		$dataman =& new vB_Datamanager_Blog_Category($vbulletin, vB_DataManager_Constants::ERRTYPE_CP);
		...
		$dataman->set_condition("CONCAT(',', parentlist, ',') LIKE '%," . $vbulletin->GPC['blogcategoryid'] . ",%'");
		$dataman->delete();

		build_category_permissions();

		print_stop_message2('deleted_blog_category_successfully', 'blog_admin', array('do'=>'listcat'));
	}
}
...
if ($_REQUEST['do'] == 'listcp')
{
	...
	$categorypermissions = $assertor->getRows('getCategoryPermissions', array(
			vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED
	));
...
{
	$vbulletin->input->clean_array_gpc('r', array(
		'categorypermissionid' => vB_Cleaner::TYPE_UINT,
		'blogcategoryid'       => vB_Cleaner::TYPE_UINT,
	...
	{
		$getperms = $assertor->getRow('getCategoryPermissionsById', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'categorypermissionid' => $vbulletin->GPC['categorypermissionid']
		));
	...
	{
		$category = $assertor->getRow('getCategoryTitle', array(
				vB_dB_Query::TYPE_KEY => vB_dB_Query::QUERY_STORED,
				'category' => $vbulletin->GPC['blogcategoryid']
		));
		$usergroup = $assertor->getRow('usergroup', array('usergroupid' => $vbulletin->GPC['usergroupid']));
		$permsgetter_ = 'usergroup permissions';

		$getperms = $assertor->getRow('usergroup', array('usergroupid' => $vbulletin->GPC['usergroupid']));
	...
	print_description_row('
		<label for="uug_1"><input type="radio" name="useusergroup" value="1" id="uug_1" tabindex="1" onclick="this.form.reset(); this.checked=true;"' . (!$vbulletin->GPC['categorypermissionid'] ? ' checked="checked"' : '') . ' />' . $vbphrase['use_default_permissions_gcppermission'] . '</label>
		<br />
...
if ($_POST['do'] == 'updatecp')
{
	$vbulletin->input->clean_array_gpc('p', array(
		...
		'categorypermission'   => vB_Cleaner::TYPE_ARRAY,
	));
	...
	if ($vbulletin->GPC['useusergroup'])
	{
		...
		if ($vbulletin->GPC['categorypermissionid'])
		{
			$assertor->delete('blog_categorypermission', array('categorypermissionid' => $vbulletin->GPC['categorypermissionid']));

			build_category_permissions();
			print_stop_message2('deleted_category_permissions_successfully','blog_admin', array('do'=>"listcp#category" . $vbulletin->GPC['blogcategoryid']));
		}
		else
		{
			build_category_permissions();
			print_stop_message2('saved_category_permissions_successfully','blog_admin', array('do'=>"listcp#category" . $vbulletin->GPC['blogcategoryid']));
		}
	}
	else
	{
...
if ($_REQUEST['do'] == 'stats')
{
	...
	{
		if (!($userexist = $assertor->getRow('user', array(
				'username' => $vbulletin->GPC['username']
		))))
		{
			print_stop_message2('invalid_user_specified');
	...
	{
		$vbulletin->GPC['start'] = TIMENOW - 3600 * 24 * 30;
		$params['start'] = $vbulletin->GPC['start'];
	}

	if (empty($vbulletin->GPC['end']))
	{
		$vbulletin->GPC['end'] = TIMENOW;
		$params['end'] = $vbulletin->GPC['end'];
	}
	...
	if (!empty($vbulletin->GPC['scope']))
	{
		...
		$params['phpformat']	= $phpformat;
		$params['start_time'] = $start_time;
		$params['end_time']		= $end_time;
		$params['userid']			= (!empty($userexist['userid']) ? $userexist['userid'] : "");
		$params['nullvalue']	= $vbulletin->GPC['nullvalue'];

		$statistics = $assertor->getRows('getBlogUserSummaryStats', $params);

		$dates = $results = array();
		while ($stats = $vbulletin->db->fetch_array($statistics))
		...
		$style = $assertor->getRow('style', array(
...
if ($_POST['do'] == 'taginsert')
{
	...
	if ($vbulletin->db->query_first("
		...
		WHERE tagtext = '" . $vbulletin->db->escape_string($vbulletin->GPC['tagtext']) . "'
	"))
	{
		print_stop_message2('tag_exists');
	}
	...
	if ($errors)
	{
		print_stop_message2(array('generic_error_x', implode('<br /><br />', $errors)));
	}
	...
	if ($vbulletin->GPC['tagtext'])
	{
		$vbulletin->db->query_write("
			INSERT IGNORE INTO " . TABLE_PREFIX . "blog_tag
				(tagtext, dateline)
			VALUES
				('" . $vbulletin->db->escape_string($vbulletin->GPC['tagtext']) . "', " . TIMENOW . ")
		");
	}

	print_stop_message2('tag_saved','blog_admin', array('do'=>'tags','pagenumber'=>$vbulletin->GPC['pagenumber'],'pp'=>$vbulletin->GPC['perpage'] , 'sort' => $vbulletin->GPC['sort']));
}
...
if ($_REQUEST['do'] == 'tags')
{
	...
	if (empty($vbulletin->GPC['delete']))
	{
		...
		do
		{
			...
			$tag_count = $vbulletin->db->found_rows();
			if ($start >= $tag_count)
			{
...
if ($_POST['do'] == 'tagkill')
{
	...
	if ($vbulletin->GPC['tag'])
	{
		...
		if (!empty($tagstodelete))
		{
			...
			while ($blog = $vbulletin->db->fetch_array($entries_result))
			{
				...
				if ($newtags_string != $thread['taglist'])
				{
					...
					$blogdm =& datamanager_init('Blog', $vbulletin, vB_DataManager_Constants::ERRTYPE_SILENT, 'blog');
		...
		$vbulletin->db->query_write("UPDATE " . TABLE_PREFIX . "blog_user SET tagcloud = ''");
	}

	print_stop_message2('tags_edited_successfully''blog_admin', array('do'=>'tags','pagenumber'=>$vbulletin->GPC['pagenumber'],'pp'=>$vbulletin->GPC['perpage'] , 'sort' => $vbulletin->GPC['sort']));
}
*/
// ########################################################################
...
if ($_POST['do'] == 'updateusercss')
{
	...
	{
		print_stop_message2('invalid_values_customize_blog');
	}

	$usercss->save();

	print_stop_message2('saved_blog_customizations_successfully','user', array('do'=>'edit','u'=>$userinfo['userid']));
}
...
if ($_REQUEST['do'] == 'usercss')
{
	...
	?>
	<script type="text/javascript" src="' . $vbulletin->options['bburl'] . '/clientscript/vbulletin_cpcolorpicker.js?v=<?php echo SIMPLE_VERSION; ?>"></script>
	<?php
