...
function fetch_online_status(&$user)
{
	...
	{
		$currentUser = vB_User::fetchUserinfo();
	...
	if ($user['lastactivity'] > $datecut AND $user['lastvisit'] != $user['lastactivity'])
	{
		...
		{
			if (
					vB::getCurrentSession()->get('userid') == $user['userid'] OR 
					vB::getUserContext()->hasPermission('genericpermissions','canseehidden')
				)
			{
